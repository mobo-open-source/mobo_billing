...
class CustomerProvider extends ChangeNotifier {
      ...
      final countResult = await _apiService
          ...
          .timeout(
            ...
            onTimeout: () => throw TimeoutException(
              'Loading customers timed out after 30 seconds',
            ),
          );
    ...
    try {
      ...
      final result = await _apiService
          ...
          .timeout(
            ...
            onTimeout: () => throw TimeoutException(
              'Loading group contacts timed out after 60 seconds',
            ),
          );
    ...
    try {
      final result = await _apiService
          ...
          .timeout(
            ...
            onTimeout: () => throw TimeoutException(
              'Fetching customer details timed out after 15 seconds',
            ),
          );
  ...
  Future<int> createCustomer(Map<String, dynamic> customerData) async {
    try {
      ...
      _error = null;
      notifyListeners();
      final id = await _apiService
          .create('res.partner', customerData)
          .timeout(
            const Duration(seconds: 20),
            onTimeout: () => throw TimeoutException(
              'Creating customer timed out after 20 seconds',
            ),
          );
      await loadCustomers();
      return id;
  ...
  Future<bool> updateCustomer(int id, Map<String, dynamic> customerData) async {
    try {
      ...
      final success = await _apiService
          .write('res.partner', [id], customerData)
          .timeout(
            const Duration(seconds: 20),
            onTimeout: () =>
                throw TimeoutException('Updating customer timed out'),
          );
      if (success) {
  ...
  Future<Map<String, dynamic>> getCustomerStats(int partnerId) async {
    try {
      ...
      final companyFilter =
          (session != null && session.allowedCompanyIds.isNotEmpty)
          ? ['company_id', 'in', session.allowedCompanyIds]
          : null;

      List<dynamic> ordersDomain = [
        ['partner_id', '=', partnerId],
      ];
      if (companyFilter != null) ordersDomain.add(companyFilter);

      final ordersResult = await _apiService
          .call('sale.order', 'search_count', [ordersDomain])
          .timeout(
            const Duration(seconds: 20),
            onTimeout: () =>
                throw TimeoutException('Loading order count timed out'),
          );
      ...
      final totalAmountResult = await _apiService
          .call(
            'sale.order',
            'search_read',
            [amountDomain],
          ...
          .timeout(
            const Duration(seconds: 20),
            onTimeout: () =>
                throw TimeoutException('Loading total amount timed out'),
          );
      ...
      final confirmedOrdersResult = await _apiService
          .call('sale.order', 'search_count', [confirmedDomain])
          .timeout(
            const Duration(seconds: 20),
            onTimeout: () => throw TimeoutException(
              'Loading confirmed orders count timed out',
            ),
          );
      ...
      final draftOrdersResult = await _apiService
          .call('sale.order', 'search_count', [draftDomain])
          .timeout(
            const Duration(seconds: 20),
            onTimeout: () =>
                throw TimeoutException('Loading draft orders count timed out'),
          );
    ...
    try {
      return await _apiService
          ...
          .timeout(
            const Duration(seconds: 15),
            onTimeout: () =>
                throw TimeoutException('Archiving customer timed out'),
          );
    } catch (e) {
      rethrow;
    }
  ...
  Future<bool> deleteCustomer(int customerId) async {
    try {
      final result = await _apiService
          .unlink('res.partner', [customerId])
          .timeout(
            const Duration(seconds: 15),
            onTimeout: () =>
                throw TimeoutException('Deleting customer timed out'),
          );
      if (result) {
    ...
    try {
      final result = await _apiService
          ...
          .timeout(
            const Duration(seconds: 20),
            onTimeout: () =>
                throw TimeoutException('Geolocalizing customer timed out'),
          );
      return result == true;
    } catch (e) {
      rethrow;
