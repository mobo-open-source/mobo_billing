...
class _ProfileDetailsScreenState extends State<ProfileDetailsScreen> {
  ...
  static const String _cacheKeyUserBase = 'user_profile';
  static const String _cacheKeyUserWriteDateBase = 'user_profile_write_date';

  String _getEffectiveCacheKey(String base) {
    ...
    return '${base}_${cleanDb}_${session.userId}';
  }
    ...
    try {
      ...
      if (result is List && result.isNotEmpty && mounted) {
        ...
        try {
          ...
          final writeDate = data['write_date'];
          if (writeDate != null) {
            final dateKey = _getEffectiveCacheKey(_cacheKeyUserWriteDateBase);
            await prefs.setString(dateKey, writeDate.toString());
          }
        } catch (e) {}
      } else {
        if (mounted) {
  ...
  Future<void> _saveAllChanges() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        settingsProvider.fetchUserProfile().catchError((e) {});
      }
    } catch (e) {
  ...
  bool _isValidImage(dynamic data) {
    ...
    if (data is String) {
      ...
      try {
        ...
        if (dataUrlSvgUtf8.hasMatch(raw)) return true;

        if (raw.trimLeft().startsWith('<svg')) return true;
        ...
        if (cleanBase64.isNotEmpty) {
          ...
          if (_looksLikeImage(bytes)) return true;

          try {
            final text = String.fromCharCodes(bytes);
            if (text.trimLeft().startsWith('<svg')) return true;
          } catch (_) {}
        }
      } catch (e) {
        return false;
        ...
        children: [
          GestureDetector(
            onTap: _isEditMode
                ? null
                : () {
                    final pickedValid = _isValidImage(_pickedImageBase64);
                    final remoteValid = _isValidImage(_userData?['image_1920']);

                    if (pickedValid || remoteValid) {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                ...
                child: ListView(
                  ...
                  children: const [
                    ...
                    EmptyStateWidget(
                      ...
                      subtitle:
                          'We couldn\'t load your profile. Pull down to refresh or try again later.',
                    ),
                    SizedBox(height: 120),
                  ],
                ),
