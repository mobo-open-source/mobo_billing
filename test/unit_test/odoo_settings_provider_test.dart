...
void main() {
  ...
  group('OdooSettingsProvider Tests', () {
    test('fetchInvoiceSettings should load settings from network', () async {
      when(
        () => mockApiService.searchRead('res.company', any(), any(), 0, 1),
      ).thenAnswer(
        (_) async => [
          {
            'id': 1,
      ...
      when(
        () => mockApiService.searchRead('res.currency', any(), any(), 0, 1),
      ).thenAnswer(
        (_) async => [
          {
            'name': 'USD',
      ...
      when(
        () => mockApiService.searchRead('account.tax', any(), any()),
      ).thenAnswer((_) async => []);

      when(
        () => mockApiService.searchRead('account.payment.term', any(), any()),
      ).thenAnswer((_) async => []);

      when(
        () => mockApiService.searchRead('account.journal', any(), any()),
      ).thenAnswer((_) async => []);
